...
public:
	...
	TArray<class UGMCheatSystem*>                      GMCheatSystems;                                           // 0x00D8(0x0010) (ZeroConstructor)
	class AActor*                                      PVETestMonster;                                           // 0x00E8(0x0008) (ZeroConstructor, IsPlainOldData)
	unsigned char                                      UnknownData01[0x30];                                      // 0x00F0(0x0030) MISSED OFFSET
	TArray<class ASkeletalMeshActor*>                  SkeletalMeshActorPool;                                    // 0x0120(0x0010) (ZeroConstructor)
...
public:
	...
	void SwitchUIWidgetShowFromUserWidget(const struct FName& ContainName, const struct FString& UserWidgetName, const struct FString& WidgetName);
	void SwitchUIWidgetShow(const struct FName& ContainName, const struct FString& WidgetName);
	void SwitchUIContainsShow(const struct FName& Name);
	bool IsPublishVersion();
	int GetHZBValue();
	float GetDistanceCullingFactor();
};
