...
int32_t hook_onInputEvent(struct android_app* app, AInputEvent* event) {
    if (g_ImGuiInitialized && event) {
        ...
        ImGuiIO &io = ImGui::GetIO();
        if (g_MenuOpen && handled && io.WantCaptureMouse) return 1;
    }
    if (orig_onInputEvent) return orig_onInputEvent(app, event);
    return 0;
}

int hook_AInputQueue_getEvent(AInputQueue* queue, AInputEvent** outEvent) {
    int result = -1;
    if (orig_AInputQueue_getEvent) result = orig_AInputQueue_getEvent(queue, outEvent);
...
void DrawMenu() {
    ...
    drawCount++;
    if (drawCount < 10 || drawCount % 100 == 0) LOGI("[ImGui] DrawMenu count %d tab=%d open=%d", drawCount, g_MenuTab, g_MenuOpen);
    if (drawCount <= 20) {
        ...
        if (bg) {
            ...
            bg->AddText(ImVec2(110, 110), IM_COL32(255, 255, 255, 255), "SANKE TEST - BACKGROUND GREEN");
        }
    }
    if (!g_MenuOpen) {
        ...
        if (ImGui::Button("Show Menu", ImVec2(180, 50))) g_MenuOpen = true;
        ImGui::End();
        return;
    }
    ImGuiIO &io = ImGui::GetIO();
    ...
    char titleBuf[128];
    sprintf(titleBuf, "SANKE MENU DEBUG FPS %.1f - VISIBLE AT 0,0", io.Framerate);
    if (ImGui::Begin(titleBuf, &g_MenuOpen, ImGuiWindowFlags_NoScrollbar)) {
        ...
        ImGui::SameLine();
        if (ImGui::Button("Test Button", ImVec2(200, 50))) LOGI("[ImGui] Test button clicked");
        ImGui::Separator();
        ...
        if (g_MenuTab == 0) {
            ...
            if (ImGui::Checkbox("VisCheck (Shared)", &vis)) { Cheat::Aimbot::VisCheck = vis; Cheat::BulletTrack::VisCheck = vis; }
            bool knock = Cheat::Aimbot::IgnoreKnock;
            if (ImGui::Checkbox("Ignore Knock (Shared)", &knock)) { Cheat::Aimbot::IgnoreKnock = knock; Cheat::BulletTrack::IgnoreKnock = knock; }
            bool bot = Cheat::Aimbot::IgnoreBot;
            if (ImGui::Checkbox("Ignore Bot (Shared)", &bot)) { Cheat::Aimbot::IgnoreBot = bot; Cheat::BulletTrack::iGnoreBot = bot; }
            ImGui::Separator();
            ...
            if (ImGui::RadioButton("Head", hitTarget == 0)) { Cheat::BulletTrack::HitTarget = 0; Cheat::BulletTrack::HitWhere = false; }
            ImGui::SameLine();
            if (ImGui::RadioButton("Body", hitTarget == 1)) { Cheat::BulletTrack::HitTarget = 1; Cheat::BulletTrack::HitWhere = true; }
            ImGui::Separator();
            ImGui::Text("Adjust Accuracy - 3 Modes");
            int acc = Cheat::BulletTrack::AccuracyMode;
            if (ImGui::RadioButton("1 Bullet / 3 (skip 2)", acc == 0)) { Cheat::BulletTrack::AccuracyMode = 0; Cheat::BulletTrack::HitChance = false; Cheat::BulletTrack::SBullet = false; }
            if (ImGui::RadioButton("2 Bullets / 3 (skip 1)", acc == 1)) { Cheat::BulletTrack::AccuracyMode = 1; Cheat::BulletTrack::HitChance = false; Cheat::BulletTrack::SBullet = true; }
            if (ImGui::RadioButton("Full Track - Every Bullet", acc == 2)) { Cheat::BulletTrack::AccuracyMode = 2; Cheat::BulletTrack::HitChance = true; Cheat::BulletTrack::SBullet = true; }
            ImGui::Separator();
            bool vis = Cheat::BulletTrack::VisCheck;
            if (ImGui::Checkbox("Visibility Check (Shared)", &vis)) { Cheat::BulletTrack::VisCheck = vis; Cheat::Aimbot::VisCheck = vis; }
            bool knock = Cheat::BulletTrack::IgnoreKnock;
            if (ImGui::Checkbox("Ignore Knocked (Shared)", &knock)) { Cheat::BulletTrack::IgnoreKnock = knock; Cheat::Aimbot::IgnoreKnock = knock; }
            bool bot = Cheat::BulletTrack::iGnoreBot;
            if (ImGui::Checkbox("Ignore Bot (Shared)", &bot)) { Cheat::BulletTrack::iGnoreBot = bot; Cheat::Aimbot::IgnoreBot = bot; }
            ImGui::Separator();
            ImGui::Checkbox("Show Circle", &Cheat::FOV::ShowCircle);
            ImGui::ColorEdit4("Circle Color", (float*)&Cheat::FOV::CircleColor);
        } else if (g_MenuTab == 3) {
            ...
            ImGui::Text("Item ESP");
            ImGui::Separator();
            if (ImGui::Button("Enable All")) for (auto &p : Items) p.second = true;
            ImGui::SameLine();
            if (ImGui::Button("Disable All")) for (auto &p : Items) p.second = false;
            ImGui::Separator();
            ...
            for (auto &cat : items_data) {
                ...
                if (ImGui::CollapsingHeader(catName.c_str())) {
                    for (auto &it : cat["Items"]) {
                        ...
                        if (ImGui::Checkbox(label.c_str(), &enabled)) Items[id] = enabled;
                    }
                }
            }
            ImGui::EndChild();
            ...
            ImGui::Text("Density: %.0f", density);
            ImGui::Text("Hooks: ReceiveDrawHUD 0xafc6044, AInputQueue 0xD494B60, ShootBulletInner 0x6ff841c, real eglSwapBuffers");
        }
        ImGui::EndChild();
    }
    ImGui::End();
...
void RenderImGui() {
    ...
    frameCount++;
    if (frameCount < 20 || frameCount % 50 == 0) LOGI("[ImGui] RenderImGui frame %d g_MenuOpen=%d w=%d h=%d", frameCount, g_MenuOpen, glWidth, glHeight);
    ImGuiIO &io = ImGui::GetIO();
    io.DisplaySize = ImVec2((float)glWidth, (float)glHeight);
    io.DisplayFramebufferScale = ImVec2(1.0f, 1.0f);
    ...
    if (err != GL_NO_ERROR) LOGI("[ImGui] GL error after RenderDrawData: 0x%x frame=%d", err, frameCount);
}
...
EGLBoolean hook_eglSwapBuffers_Offset(EGLDisplay dpy, EGLSurface surface) {
    ...
    g_InEglSwap = true;
    if (g_SwapDepth > 1) {
        LOGI("[ImGui] offset hook reentrant depth=%d", g_SwapDepth);
    ...
    g_SwapDepth--;
    if (g_SwapDepth == 0) g_InEglSwap = false;
    if (g_SwapCount < 20) LOGI("[ImGui] offset hook after orig res=%d count=%d", res, g_SwapCount);
    return res;
}
...
EGLBoolean hook_eglSwapBuffers_Real(EGLDisplay dpy, EGLSurface surface) {
    ...
    g_InEglSwap = true;
    if (g_SwapDepth > 1) {
        LOGI("[ImGui] real hook reentrant depth=%d", g_SwapDepth);
        EGLBoolean res = EGL_FALSE;
...
EGLBoolean hook_eglSwapBuffers(EGLDisplay dpy, EGLSurface surface) { return hook_eglSwapBuffers_Offset(dpy, surface); }
...
__int64 hook_renderer(__int64 *ConfigAttrib, int a2) {
    ...
    if (!g_ImGuiInitialized) {
        ...
        EGLSurface surface = reinterpret_cast<EGLSurface>(*(uintptr_t *) (*ConfigAttrib + 120LL));
        EGLContext ctx = eglGetCurrentContext();
        if (dpy != EGL_NO_DISPLAY && surface != EGL_NO_SURFACE && ctx != EGL_NO_CONTEXT) InitImGui(dpy, surface, g_App->window);
    }
    return ret;
}
...
void InstallRendererHook(uintptr_t rendererAddr) {
    ...
    if (stub) LOGI("Renderer hook installed at %p", (void*)rendererAddr);
    else LOGI("Renderer hook failed err=%d", shadowhook_get_errno());
}
...
void InstallImGuiHooks() {
    if (Cheat::libUE4Base != 0) {
        ...
        LOGI("[ImGui] Skipping offset eglSwapBuffers hook at %p (causes hang), using real libEGL only", (void*)eglAddr);
        unsigned char* bytes = (unsigned char*)eglAddr;
        LOGI("[ImGui] offset bytes: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x",
    ...
    for (int i = 0; eglLibs[i] != nullptr; ++i) {
        ...
        if (!sym) continue;
        if (orig_eglSwapBuffers_Real && sym == (void*)orig_eglSwapBuffers_Real) continue;
        LOGI("[ImGui] Hooking real eglSwapBuffers in %s @ %p", eglLibs[i], sym);
        ...
        if (stub) {
            ...
            LOGI("[ImGui] real eglSwapBuffers hooked in %s stub=%p orig=%p", eglLibs[i], stub, orig_eglSwapBuffers_Real);
            orig_eglSwapBuffers = orig_eglSwapBuffers_Real;
            orig_eglSwapBuffers_Offset = orig_eglSwapBuffers_Real;
            g_EglHookInstalled = true;
        } else {
            int dRet = DobbyHook(sym, (void*)hook_eglSwapBuffers_Real, (void**)&orig_eglSwapBuffers_Real);
            if (dRet == 0) {
                g_EglRealHookInstalled = true;
                LOGI("[ImGui] real eglSwapBuffers hooked via Dobby in %s orig=%p", eglLibs[i], orig_eglSwapBuffers_Real);
                orig_eglSwapBuffers = orig_eglSwapBuffers_Real;
                g_EglHookInstalled = true;
            }
        }
    }
    if (!g_EglHookInstalled) LOGI("[ImGui] All eglSwapBuffers hooks failed");
    else LOGI("[ImGui] eglSwapBuffers hook installed: real=%d (offset skipped)", g_EglRealHookInstalled);
    InstallInputHooks();
}
