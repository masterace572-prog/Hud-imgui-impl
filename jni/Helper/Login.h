...
inline std::string Login(const char *userKey) {
    ...
    //======LINKURL=====//
       if (curl) {
        ...
        std::string sRedLink = OBFUSCATE("https://cheathub.ai-new.xyz/server");
		curl_easy_setopt(curl, CURLOPT_URL, sRedLink.c_str());
  //      curl_easy_setopt(curl, CURLOPT_PINNEDPUBLICKEY, "sha256//GOdv5BzO0XXuBq/Afp/CtuQPUuccQttfwjwronP+KBE=");
  ...
  char data[4096];
		...
		std::string tryout = OBFUSCATE("sha256//sWCVt8UKeQudc9VCn87OcuWSTNsVwoULQwSTH2XJveM=");
        ...
        if (res == CURLE_OK) {
            try {
                ...
                if (result[StrEnc("(>_LBm", "\x5B\x4A\x3E\x38\x37\x1E", 6).c_str()] == true) {
					...
					//	servername = result["data"]["servername"].get<std::string>();
                        if (bValid) {
                            pthread_t ptid;
                        }
                    }
